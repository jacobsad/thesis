...
Introducing mutation dramatically increases the complexity of the model. An example of this can be seen in figure~\ref{fig:s3}, where a high level of mutation introduces temporary `comebacks' in the level of cooperation in the labelled regions A and B. However, 

<<s3,tidy=FALSE, echo=FALSE, fig.align='center', fig.height=3.75, fig.width=5, out.height='0.75\\textwidth', out.width='\\textwidth', fig.cap=paste('Comebacks. When mutation rates are high, it is possible for unsustainable levels of cooperation to emerge for short periods of time: see in particular the labelled regions A and B. $N = ', N, '$, $m = ', mortality, '$, $\\mu = ', std_dev, '$, $shape = (', beta_1, ',', beta_2, ')$.', sep = ""), fig.env='marginfigure', dev='tikz'>>=
...
\noindent One consequence of moving away from non-pure strategies is that the timescale increases dramatically.