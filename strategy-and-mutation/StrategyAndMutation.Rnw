...
%=====================================================================================================================%
<<beta-dist, tidy=FALSE, echo=FALSE, warning=FALSE, fig.align='center', out.width='\\textwidth', out.height='0.75\\textwidth', fig.cap=paste('Probability densities for various shapes of the Beta distribution. This distribution allows a wide variety of initial conditions for the level of cooperation.', sep = ""), fig.env='figure'>>=
...
ggplot(df, aes(x = x, y = n)) + geom_line(aes(color = shape)) + xlab('p') + ylab('Probability Density') + ylim(c(0, 2.5)) + 
    theme(legend.position="left", 
          #axis.title.y = element_text(family = "Gill Sans MT", size = 18), 
          axis.title.y = element_blank(),
          axis.title.x = element_text(family = "Helvetica", face = "italic", size = 18),
          #legend.title = element_text(family = "Gill Sans MT", size = 18, face = "plain", hjust = 0.5),
...
%=====================================================================================================================%
<<a-pop-dens, tidy=FALSE, echo=FALSE, warning=TRUE, fig.align='center', out.width='2in', out.height='2.2in', fig.cap=paste('Probability density for scenario (a) in red. Notice that it is almost identical to the uniform distribution.', sep = ""), fig.env='marginfigure', fig.pos='-2in'>>=
ggplot(data.frame(a = population), aes(x = a)) + stat_density(kernel="biweight", aes(x = a, fill = "red", alpha = 0.8)) + 
    xlab('p') + ylab('Probability Density') + ylim(0, 2.5) +
...
\noindent The results are visible in figure~\ref{fig:beta-pop-dens}.

% Draws EVEN MORE beta densities
%=====================================================================================================================%
<<beta-pop-dens, tidy=FALSE, echo=FALSE, warning=TRUE, fig.align='center', out.width='2in', out.height='2.2in', fig.cap=paste('Probability densities for scenarios (b), (c), and (d) in blue, green, and red respectively.', sep = ""), fig.env='marginfigure', fig.pos='-2in'>>=
pop + xlab('p') + ylab('Probability Density') + 
      theme(legend.position="none", 
            axis.title.y = element_text(family = "Helvetica", size = 18),
            axis.title.x = element_text(family = "Helvetica", face = "italic", size = 18))
@
...
In order for the population to maintain variation among individuals, inheritance must be subject to mutation. In fact, in the context of the evolution of cooperation the presence of a mechanism to preserve varation (usually, mutation) appears to be paramount\autocite{McNamara2004}.